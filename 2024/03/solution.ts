 ...
 * `mul(44,46)` multiplies 44 by 46 to get a result of 2024. Similarly, `mul(123,4)` would multiply
 * 123 by 4.
 *
 ...
 * instruction produces 161 (2_4 + 5_5 + 11_8 + 8_5).
 *
 ...
 * instructions. Only the most recent do() or don't() instruction applies. At the beginning of the
 ...
 * other mul instructions function normally, including the one at the end that gets re-enabled by a
 * do() instruction.
 *
 * This time, the sum of the results is 48 (2_4 + 8_5).
 *
...
function partOne(input: string) {
	...
	console.log(
		matches
			.toArray()
			.map((match) => (match.groups ? Number(match.groups["n1"]) * Number(match.groups["n2"]) : 0))
...
function partTwo(input: string) {
	...
	const regex = /mul\((?<n1>\d+),(?<n2>\d+)\)|do\(\)|don't\(\)/g;
